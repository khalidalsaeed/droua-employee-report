...
/* محافظٌ وحتمي: كل خطوة تُزيل اختلافًا في **الكتابة** لا في **الهوية**.
   ...
   ولا يُدخَل فيه مرادف لم يُرَ في مستند حقيقي وسجلّ حقيقي معًا: فبنكٌ
   يطبع اسمه في الإيصال بصيغة وفي السجلّ بأخرى يبقى `unlinked` حتى
   يُضاف مرادفه بعد تحقّق — وهذا الفشل الآمن هو المقصود لا نقصٌ فيه. */
/* مرادفان اثنان لا غير، وكلاهما مُثبَت على بيانات حقيقية بالتحقّق
   المحلّي (‎scripts/validate-receipt-linking.js): الإيصال يطبع اسم البنك
   لاتينيًا والسجلّ يحمله عربيًا، فالصيغتان للمؤسّسة نفسها يقينًا لا
   احتمالًا — سبعة إيصالات وإيصالان على الترتيب.

   المفتاح والقيمة **صيغتان مُطبَّعتان** لا نصّان خامان: التطبيع يجري
   أوّلًا ثم يُستبدَل المفتاح، فلا يعتمد المرادف على فراغٍ ولا ترقيم ولا
   صورة عرض.

   ولا يُضاف هنا اسمٌ إلا بعد أن يُرى في مستند حقيقي وسجلّ حقيقي معًا.
   وليس هذا تشدّدًا: مرادفٌ خاطئ يوحّد بنكين مختلفين، فيصير رقم حساب
   يحمله شخصان مفتاحًا واحدًا — وذاك بالضبط ما بُنيت القاعدة لمنعه. */
const BANK_ALIASES = Object.freeze({
  ARABNATIONALBANK: "البنكالعربيالوطني",
  ALINMABANK: "مصرفالانماء",
});
