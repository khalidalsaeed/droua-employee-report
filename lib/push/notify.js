...
const { findByIdRaw, findByEmailRaw } = require("../auth/users");
...
/* ─── إشعار إنشاء مسير الرواتب الشهري ───

   جمهوره ليس جمهور تنبيهات الانتهاء: هذا التنبيه موجَّه إلى شخص واحد
   بعينه — من ينتظره إكمال المسير — لا إلى كل من يملك صلاحية. لذلك لا
   يمرّ على eligibleSubscriptions() بل على أجهزة صاحب العنوان المُهيّأ
   في PAYROLL_NOTIFY_EMAIL وحده.

   فحص payroll:view يبقى قائمًا فوق ذلك كطبقة ثانية: لو جُرّد ذلك الحساب
   من صلاحية الرواتب فلا يصحّ أن يستمرّ إشعارٌ عن مسير لا يستطيع فتحه.

   الحاجز هو الحاجز نفسه المستعمل للتنبيه اليومي — claim/settle/release
   على push_notification_log بمفتاح payroll:2026-08 — فلا آلية ثانية
   لعدم التكرار في المنصّة. */
async function subscriptionsForEmail(email) {
  const address = String(email || "").trim();
  if (!address) return { subs: [], reason: "no_address" };
  const user = await findByEmailRaw(address);
  if (!user) return { subs: [], reason: "user_not_found" };
  if (user.status !== "active") return { subs: [], reason: "user_inactive" };
  if (!hasPermission(user, "payroll", "view")) return { subs: [], reason: "user_lacks_permission" };
  return { subs: await subscriptions.listForUser(user.id), reason: null, userId: user.id };
}

/* `run` يكفي منه {id, monthLabel}. */
async function sendPayrollRunCreated(run) {
  if (!isConfigured()) return { ok: true, skipped: "vapid_not_configured", sent: 0 };

  const { notifyEmailAddress, buildPush, dedupKey } = require("../payroll/notify");
  const { subs, reason } = await subscriptionsForEmail(notifyEmailAddress());
  /* لا جهاز مسجَّل ليس خطأً: البريد وصل، وهذه القناة ببساطة لا وجهة لها
     على هذا الحساب بعد. */
  if (!subs.length) return { ok: true, sent: 0, devices: 0, skipped: reason || "no_devices" };

  const key = dedupKey(run.id);
  const payload = buildPush(run);

  let sent = 0, skipped = 0, gone = 0, failed = 0, lastError = null;
  for (const sub of subs) {
    const logId = await claim(sub.id, key);
    if (!logId) { skipped++; continue; }          // وصل هذا الجهاز مسبقًا
    const res = await sendToSubscription(sub, payload);
    if (res.ok) {
      await settleClaim(logId, "sent", null);
      sent++;
  ...
  /* skipped يعني «وصل هذا الجهاز مسبقًا» لا فشلًا، فالنجاح يُقاس بغياب
     الفشل لا بوجود إرسال: تشغيل ثانٍ في اليوم نفسه كله skipped وهو
     النتيجة الصحيحة تمامًا. */
  return { ok: failed === 0, devices: subs.length, sent, skipped, gone, failed, error: failed ? lastError : null, dedupKey: key };
}
...
module.exports = {
  ...
  sendPayrollRunCreated, subscriptionsForEmail,
};
