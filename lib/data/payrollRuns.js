...
const PROOF_COLS = `employee_eid, employee_name, job_title, file_url, file_name, to_char(uploaded_at AT TIME ZONE 'Asia/Riyadh','YYYY-MM-DD') as uploaded_at, uploaded_by`;
...
async function proofsFor(sql, runId) {
  return sql`SELECT ${sql.unsafe(PROOF_COLS)} FROM payroll_transfer_proofs WHERE run_id = ${runId} ORDER BY id`;
}
...
async function list() {
  ...
  const allProofs = runs.length
    ? await sql`SELECT run_id, ${sql.unsafe(PROOF_COLS)} FROM payroll_transfer_proofs ORDER BY id`
    : [];
