...
async function update(id, patch) {
  ...
  if (Array.isArray(patch.attachments)) {
    for (const a of patch.attachments) {
      ...
      await upsertAttachment(sql, id, { ...base, ...a });
    }
  }
  return get(id);
}
...
async function remove(id) {
  const sql = getSql();
  const rows = await sql`DELETE FROM payroll_runs WHERE id = ${id} RETURNING id`;
  if (!rows[0]) throw new Error("مسير الرواتب غير موجود");
}
