...
async function listFindings(sql, runId, { includeResolved = false } = {}) {
  ...
  const rows = includeResolved
    ? await sql`SELECT * FROM droua_payroll_findings WHERE run_id = ${runId}::uuid
        ORDER BY severity DESC, employee_ref NULLS FIRST`
    : await sql`SELECT * FROM droua_payroll_findings WHERE run_id = ${runId}::uuid
        AND resolved_at IS NULL ORDER BY severity DESC, employee_ref NULLS FIRST`;
...
module.exports = {
  ...
  STATUSES, STATUS_LABELS, SEVERITIES, SCOPES, MAX_NOTE,
};
