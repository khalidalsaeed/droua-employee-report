...
async function downloadFile(req, res, { sql, ctx, params }) {
  ...
  try {
    ...
    /* فشل السلامة سُجّل داخليًّا في files.readFile. وما يخرج هنا لا يميّز
       «غير موجود» عن «فاسد»: التمييز يقول لمن يستكشف أين يبحث. */
...
async function listFindings(req, res, { sql, params, query }) {
  const includeResolved = String(query.resolved || "") === "1";
