...
const monthSettings = require("./settings");
...
async function listRuns(req, res, { sql }) {
  ...
  return ok(res, {
    ...
    runs: rows.map((run) => ({
      ...
      /* المقام هو **اللازم** لا كل الخانات: شهرٌ مكتملٌ بلا ملفّ عمل
         إضافيّ يجب أن يُقرأ «4 / 4» لا «4 / 5» — وإلّا بدا ناقصًا أبدًا. */
      filesExpected: files.REQUIRED_KINDS.length,
...
async function getRun(req, res, { sql, params }) {
  ...
  const [slots, summary, previousRun] = await Promise.all([
    runs.fileSlots(sql, run.runId),
    findings.summary(sql, run.runId),
    runs.getByPeriod(sql, previousPeriod),
  ]);
  return ok(res, {
    run,
    slots,
    ...
    previousPeriod,
    /* «المقارنة مع 2026-07» تُقرأ وعدًا بأنها جرت. ولو لم يُرفع ذلك الشهر
       أصلًا لم تجرِ ولن تجري — فيُقال ذلك صراحةً بدل أن يُفهم العكس. */
    previousExists: Boolean(previousRun),
    findings: summary,
    /* مقامُ التغطية: الواجهة تعرضه كلّما فُتح الشهر، لا عند التحليل وحده. */
    totalRules: compare.RULES.length,
...
/* ─── إعدادات الشهر: قاسمُ الساعة والإجازات ───────────────────────────
   بيانان يُدخلهما المستخدم بيده. وكلاهما يُسجَّل في التدقيق: تعديلُ قاسمٍ
   يُغيّر مبلغًا متوقَّعًا، وإجازةٌ تُسكت ملاحظةً — وكلاهما قرارٌ يُراجَع. */

/* الشاشة تحتاج **الأسماء** لتُختار بها، والرقم الوظيفيّ يبقى المفتاح.
   فتُقرأ قائمةُ موظفي الشهر من الملفّ المرفوع، ويُقرأ ملفّ العمل الإضافي
   لاستخراج من له دقائق فعلًا.

   وفشلُ القراءة **لا يُسقط الشاشة**: تُرجَع قائمةٌ فارغة وعلمٌ يقول إنها
   لم تُحمَّل، فتعرض الواجهة الإدخال اليدويّ بديلًا. وشاشةٌ لا تفتح أسوأ
   من شاشةٍ بلا رفاهية بحث. */
async function monthRoster(sql, runId, ctx) {
  try {
    const { docs } = await analyze.loadDocs(sql, runId, ctx);
    /* قائمة الموظفين أوّلًا، ومسير الشهر بديلًا حين لا تُرفع القائمة. */
    const source = docs.employees || docs.full;
    const seen = new Map();
    for (const row of (source && source.rows) || []) {
      if (!row.empNo || seen.has(row.empNo)) continue;
      seen.set(row.empNo, { empNo: row.empNo, name: row.name || null });
    }

    const overtime = new Map();
    for (const row of (docs.overtime && docs.overtime.rows) || []) {
      if (!row.empNo) continue;
      const minutes = Object.values(row.minutes || {}).reduce((t, v) => t + Number(v || 0), 0);
      const entry = overtime.get(row.empNo) || { empNo: row.empNo, name: row.name || null, minutes: 0 };
      entry.minutes += minutes;
      if (!entry.name && row.name) entry.name = row.name;
      overtime.set(row.empNo, entry);
    }
    return {
      employees: [...seen.values()],
      /* من له دقائق فعلًا وحده: من لا عمل إضافيّ له لا قاسمَ يلزمه. */
      overtimeEmployees: [...overtime.values()].filter((e) => e.minutes > 0),
      rosterLoaded: Boolean(source),
      overtimeLoaded: Boolean(docs.overtime),
    };
  } catch (err) {
    return { employees: [], overtimeEmployees: [], rosterLoaded: false, overtimeLoaded: false };
  }
}
...
/* ─── إجماليّا الصرف ──────────────────────────────────────────────────
   ...
   المفحوص. (والمطبِّع يتخطّى صفّ المجاميع أصلًا: بلا رقمٍ ولا اسم.)

   ومسارٌ منفصل عن صفحة الشهر: فكُّ الملفّات وقراءتُها أثقل من استعلامٍ،
   فلا تُبطَّأ الصفحة كلُّها لأجل رقمين في أسفلها — ويهبط الملخّص وحده إن
   تعذّر. */
async function runTotals(req, res, { sql, ctx, params }) {
  const empty = { available: false, total: 0, count: 0 };
  let docs = null;
  try {
    ({ docs } = await analyze.loadDocs(sql, params[0], ctx));
  } catch (err) {
    return ok(res, { transfer: { ...empty }, cash: { ...empty } });
  }
  const sumOf = (doc) => {
    if (!doc) return { ...empty };
    let total = 0;
    ...
    for (const row of doc.rows || []) {
      if (!row.empNo) continue;
      total += Number(row.net || 0);
      count += 1;
    }
    /* التقريب في النتيجة وحدها — لا في كل صفّ. */
    return { available: true, total: Math.round(total * 100) / 100, count };
  };
  return ok(res, { transfer: sumOf(docs.transfer), cash: sumOf(docs.cash) });
}

async function listSettings(req, res, { sql, ctx, params }) {
  const [divisors, leaves, roster] = await Promise.all([
    monthSettings.listDivisors(sql),
    monthSettings.listLeaves(sql, params[0]),
    monthRoster(sql, params[0], ctx),
  ]);
  const saved = new Map(divisors.map((d) => [d.empNo, d.overtimeDivisor]));
  return ok(res, {
    divisors,
    leaves,
    allowedDivisors: monthSettings.DIVISORS,
    employees: roster.employees,
    /* القاسمُ المحفوظ يُرفَق بكل صفّ: الشاشة تعرضه ولا تسأل عنه ثانيةً. */
    /* المحفوظ إن وُجد، وإلّا الافتراضيّ — ومعه علمٌ يميّزهما. فالشاشة
       تعرض قيمةً جاهزة دائمًا، ويعرف المستخدم أيَّها قرارٌ وأيّها سياسة. */
    overtimeEmployees: roster.overtimeEmployees.map((e) => ({
      ...e,
      overtimeDivisor: saved.has(e.empNo)
        ? saved.get(e.empNo) : monthSettings.DEFAULT_OVERTIME_DIVISOR,
      isDefault: !saved.has(e.empNo),
    })),
    defaultDivisor: monthSettings.DEFAULT_OVERTIME_DIVISOR,
    rosterLoaded: roster.rosterLoaded,
    overtimeLoaded: roster.overtimeLoaded,
  });
}

async function putDivisorsBulk(req, res, { sql, ctx }) {
  const body = req.body || {};
  try {
    const result = await monthSettings.setDivisors(sql, body.divisors, ctx);
    await audit.log(sql, {
      event: "ot_divisors_saved", userId: ctx.actorId,
      /* عددٌ لا قائمة: التدقيق يُثبت الفعل ولا ينسخ البيانات. */
      meta: { count: result.saved },
    });
    return ok(res, result);
  } catch (err) {
    if (err.code) return bad(res, "قيمةٌ غير مقبولة");
    throw err;
  }
}

async function patchLeave(req, res, { sql, ctx, params }) {
  const body = req.body || {};
  try {
    const saved = await monthSettings.updateLeave(sql, params[0],
      { startDate: body.startDate, endDate: body.endDate });
    await audit.log(sql, {
      event: "leave_updated", userId: ctx.actorId,
      meta: { leaveId: params[0], from: saved.startDate, to: saved.endDate },
    });
    return ok(res, { leave: saved });
  } catch (err) {
    if (err.code === "not_found") return bad(res, "غير موجودة", 404);
    if (err.code) return bad(res, "بيانات إجازةٍ غير مقبولة");
    throw err;
  }
}

async function putDivisor(req, res, { sql, ctx }) {
  const body = req.body || {};
  try {
    const saved = await monthSettings.setDivisor(sql,
      { empNo: body.empNo, overtimeDivisor: body.overtimeDivisor }, ctx);
    await audit.log(sql, {
      event: "ot_divisor_set", userId: ctx.actorId,
      meta: { empNo: saved.empNo, divisor: saved.overtimeDivisor },
    });
    return ok(res, { divisor: saved });
  } catch (err) {
    if (err.code) return bad(res, "قيمةٌ غير مقبولة");
    throw err;
  }
...
async function deleteDivisor(req, res, { sql, ctx, apiPath }) {
  const empNo = decodeURIComponent(String(apiPath).replace(/^settings\/divisors\//, ""));
  try {
    const result = await monthSettings.clearDivisor(sql, empNo);
    if (!result.deleted) return bad(res, "غير موجود", 404);
    await audit.log(sql, { event: "ot_divisor_cleared", userId: ctx.actorId, meta: { empNo } });
    return ok(res, result);
  } catch (err) {
    if (err.code) return bad(res, "قيمةٌ غير مقبولة");
    throw err;
...
async function addLeave(req, res, { sql, ctx, params }) {
  const body = req.body || {};
  try {
    const saved = await monthSettings.addLeave(sql, {
      runId: params[0], empNo: body.empNo,
      startDate: body.startDate, endDate: body.endDate, note: body.note,
    }, ctx);
    await audit.log(sql, {
      event: "leave_added", userId: ctx.actorId,
      /* ⛔ لا اسم ولا سبب في التدقيق: الرقم والمدى وحدهما. */
      meta: { runId: params[0], empNo: saved.empNo, from: saved.startDate, to: saved.endDate },
    });
    return ok(res, { leave: saved });
  } catch (err) {
    if (err.code) return bad(res, "بيانات إجازةٍ غير مقبولة");
...
async function removeLeave(req, res, { sql, ctx, params }) {
  try {
    const result = await monthSettings.removeLeave(sql, params[0]);
    if (!result.deleted) return bad(res, "غير موجودة", 404);
    await audit.log(sql, { event: "leave_removed", userId: ctx.actorId, meta: { leaveId: params[0] } });
    return ok(res, result);
  } catch (err) {
    if (err.code) return bad(res, "معرّفٌ غير مقبول");
    throw err;
  }
}
...
const ROUTES = [
  ...
  ["GET", new RegExp(`^runs/${UUID_SEG}/settings$`), listSettings],
  ["GET", new RegExp(`^runs/${UUID_SEG}/totals$`), runTotals],
  ["POST", new RegExp(`^runs/${UUID_SEG}/leaves$`), addLeave],
  ["DELETE", new RegExp(`^leaves/${UUID_SEG}$`), removeLeave],
  ["PUT", /^settings\/divisors$/, putDivisor],
  ["PUT", /^settings\/divisors\/bulk$/, putDivisorsBulk],
  ["PATCH", new RegExp(`^leaves/${UUID_SEG}$`), patchLeave],
  /* الرقم الوظيفيّ في المسار: أحرفٌ وأرقامٌ بلا مسافة، كما يفرضه القيد. */
  ["DELETE", /^settings\/divisors\/[^/\s]{1,32}$/, deleteDivisor],
];
