...
async function listSettings(req, res, { sql, params }) {
  const [divisors, leaves] = await Promise.all([
    monthSettings.listDivisors(sql),
    monthSettings.listLeaves(sql, params[0]),
  ]);
  return ok(res, { divisors, leaves, allowedDivisors: monthSettings.DIVISORS });
}
