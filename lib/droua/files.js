...
/* الأنواع الأربعة بترتيب العرض. والقيم إنجليزية قصيرة لأنها تدخل الـAAD
   ومسارات الـAPI؛ والعربية للعرض وحده. */
/* ── خمس خانات، أربعٌ منها لازمة ──
   ملفّ العمل الإضافي **اختياريّ بقصد**: أشهرٌ قائمة (يوليو وأغسطس) رُفعت
   بأربعة ملفّات، ولو صار الخامس لازمًا لانقلبت كلّها «ناقصة» وظهرت لكلٍّ
   منها ملاحظةٌ حرجة عن ملفٍّ لم يكن مطلوبًا يوم أُغلقت.

   وغيابُه لا يُسكِت شيئًا: قواعد العمل الإضافي تُعلَن «لم تُقيَّم» بآليّة
   `needs` — وهو الفرق بين «فُحص ولا شيء» و«لم يُفحص». */
const KINDS = Object.freeze(["full", "transfer", "cash", "employees", "overtime"]);
const REQUIRED_KINDS = Object.freeze(["full", "transfer", "cash", "employees"]);
const KIND_LABELS = Object.freeze({
  full: "مسير الرواتب كامل",
  transfer: "مسير الرواتب تحويل",
  cash: "مسير الرواتب كاش",
  employees: "قائمة الموظفين",
  overtime: "ملفّ العمل الإضافي",
});
...
function validateInput(input) {
  ...
  /* يرمي على ما ليس في القائمة المغلقة — ونوع المحتوى يُشتقّ هنا ولا
     يُقبل من أحد. ويُعاد رميُه برمزٍ مصنَّف: الموجّه يفرّق بين خطأ مدخلات
     يُردّ 400 وعطلٍ يُردّ 500، والتفريق يقوم على الرمز لا على الرسالة. */
  let contentType;
  try {
    contentType = storage.safeContentType(format);
  } catch (err) {
    throw fail("صيغة ملفّ غير مسموحة", "bad_format");
  }
...
/* عدّ الملفّات الحالية لكل شهر في استعلام واحد. الصفحة الرئيسية تعرض اثني
   عشر شهرًا، واستعلامٌ لكلٍّ منها يعني اثنتي عشرة رحلة إلى القاعدة —
   والقاعدة هنا عبر HTTP، فالرحلة عشرات الأجزاء من الثانية لا ميكروثانية. */
async function countsByRun(sql) {
  gateContext.requireGate();
  /* اللازم وحده يُعدّ: عمود «الملفّات» في الجدول كسرٌ مقامُه أربعة، فلو
     عُدّ الاختياريّ في البسط لظهر «5 / 4». */
  const rows = await sql`SELECT run_id, count(*)::int AS present
    FROM droua_payroll_files
    WHERE superseded_at IS NULL AND kind = ANY(${REQUIRED_KINDS})
    GROUP BY run_id`;
  const out = new Map();
  for (const row of rows || []) out.set(row.run_id, Number(row.present));
  return out;
}
...
module.exports = {
  putFile, replaceFile, readFile, getFile, listCurrent, listVersions, countsByRun,
  deleteFile, findStale, sweepStale,
  KINDS, REQUIRED_KINDS, KIND_LABELS, INPUT_KEYS, CLIENT_FORBIDDEN, MAX_FILE_NAME, publicView,
};
