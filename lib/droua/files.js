...
const KINDS = Object.freeze(["full", "transfer", "cash", "employees"]);
...
async function countsByRun(sql) {
  ...
  const rows = await sql`SELECT run_id, count(*)::int AS present
    FROM droua_payroll_files WHERE superseded_at IS NULL GROUP BY run_id`;
  const out = new Map();
...
module.exports = {
  ...
  KINDS, KIND_LABELS, INPUT_KEYS, CLIENT_FORBIDDEN, MAX_FILE_NAME, publicView,
};
