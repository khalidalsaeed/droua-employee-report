...
const ALIASES = {
  ...
  /* ملفّ العمل الإضافي. «التاريخ» مفتاحُ صفٍّ لا مفتاحُ هويّة: الموظّف
     الواحد له صفٌّ لكل يوم ولكل مصدر اعتماد. */
  otDate: ["التاريخ", "تاريخ", "اليوم", "date", "day"],
  otPlannedHours: ["اجمالي الساعات المقررة", "الساعات المقررة", "المقرر", "planned hours"],
  otWorkedHours: ["اجمالي ساعات العمل", "ساعات العمل", "actual hours", "worked hours"],
  otDiff: ["الفرق", "الفارق", "difference"],
  otSource: ["المصدر", "مصدر الاعتماد", "نوع الاعتماد", "source"],
...
/* أسماءُ بند العمل الإضافي في المسير. يبقى جزءًا من البدلات ويُلتقط
   مفردًا معها. و«null» تعني «لا عمود له» — لا «صفر ريال». */
const OT_AMOUNT_RE = /(وقت|ساعات)\s*اضافي|العمل\s*الاضافي|^اضافي$|overtime/;
...
function classifyColumn(rawHeader) {
  ...
  /* معاملُ العمل الإضافي يُقرأ من الترويسة **الخام**: التطبيعُ يستبدل
     النقطة بمسافة فيصير «x1.5» ← «x1 5»، فتضيع الخمسةُ من العُشر. وملفّ
     العمل الإضافي قد يحمل أكثر من مجموعة أعمدة بالمعامل نفسه — واحدة لكل
     مصدر اعتماد — فتُجمع كلّها ولا يُؤخذ أوّلها. */
  const mult = String(rawHeader || "").match(/[x×]\s*([0-9]+(?:[.,][0-9]+)?)/i);
  if (mult) {
    const value = Number(String(mult[1]).replace(",", "."));
    if (Number.isFinite(value) && value > 0) return { kind: "otMinutes", multiplier: value };
  }
  return null;
}
...
const EXPECTED_FIELDS = {
  ...
  /* لا «صافي» هنا: ملفّ العمل الإضافي دقائقُ لا مبالغ. وقياسُه بقائمة
     المسيرات كان سيهبط بثقته إلى الصفر ويرفع علم «يحتاج مراجعة» على ملفٍّ
     سليم — وهو الخطأ نفسه الذي وقع مع ملفّ الكاش. */
  [KIND_OVERTIME]: { required: ["empNo"], useful: ["name", "otDate", "otWorkedHours"] },
};
...
function normalizeRows(kind, header, dataRows) {
  ...
  header.forEach((cell, index) => {
    ...
    if (found.kind === "otMinutes") { otCols.push({ index, label, multiplier: found.multiplier }); return; }
  ...
  if (otCols.length) {
    /* المعاملات المتاحة تُعلَن: قاعدةٌ تقارن مالًا على معاملٍ لم يُقرأ
       يجب أن تتوقّف لا أن تحسبه صفرًا. */
    mapping.otMultipliers = [...new Set(otCols.map((c) => c.multiplier))].sort((a, b) => a - b).join(" · ");
  }
  ...
  for (const raw of dataRows) {
    ...
    /* التكرار خطأٌ في المسيرات — صفّان لموظّفٍ واحد يجعلان كل مجموع
       مشكوكًا فيه. وهو **طبيعيّ** في ملفّ العمل الإضافي: صفٌّ لكل يوم
       ولكل مصدر اعتماد. فالتحذير هناك لا هنا. */
    if (kind !== KIND_OVERTIME && empNo && seen.has(empNo)) {
      warnings.push(`رقم موظّف مكرّر في الملفّ: ${empNo}`);
    }
    if (empNo) seen.add(empNo);

    const idLast4 = lastFour(at(raw, "nationalId"));

    if (kind === KIND_OVERTIME) {
      /* دقائق كل معامل تُجمع عبر **كل** أعمدته: مجموعةٌ لكل مصدر اعتماد
         (طلبات · تلقائي · يدوي)، وأخذُ أوّلها يُسقط الباقي بصمت. */
      const minutes = {};
      for (const col of otCols) {
        const value = toNumber(raw[col.index]);
        if (!value) continue;
        minutes[col.multiplier] = (minutes[col.multiplier] || 0) + value;
      }
      rows.push({
        empNo, name,
        date: cleanText(at(raw, "otDate")) || null,
        source: cleanText(at(raw, "otSource")) || null,
        plannedHours: "otPlannedHours" in map ? toNumber(at(raw, "otPlannedHours")) : null,
        workedHours: "otWorkedHours" in map ? toNumber(at(raw, "otWorkedHours")) : null,
        minutes,
      });
      continue;
    }
    ...
    /* بنودُ الخصم مفصَّلةً بأسمائها كما وردت. فـ«ارتفع الخصم 420» وحدها
       لا تُصلح شيئًا؛ و«غياب +300 وتأخير +120» تقول لصاحبها أين ينظر.
       والفارغُ يُحذف كي لا يُقارَن عمودٌ غائب بصفرٍ موجود. */
    /* بندُ العمل الإضافي داخل البدلات — ويُلتقط **معه** لا بدلًا منه:
       إخراجُه من مجموع الأجزاء يُنقص البدلات في ملفٍّ لا يُعلن مجموعها.
       ومقارنةُ العمل الإضافي بالمال تحتاجه مفردًا لا مذابًا في المجموع. */
    let otAmount = null;
    for (const col of allowanceCols) {
      if (!OT_AMOUNT_RE.test(normalizeHeader(col.label))) continue;
      otAmount = (otAmount || 0) + toNumber(raw[col.index]);
    }

    const deductionBreakdown = {};
    for (const col of deductionCols) {
      const value = toNumber(raw[col.index]);
      if (value) deductionBreakdown[col.label] = value;
    }
...
/* ── ترويسةٌ على صفّين ──
   ملفّ العمل الإضافي الحقيقيّ يضع أسماء الحقول في صفّ، ومعاملَ كل عمود
   في الصفّ الذي تحته: مجموعةٌ لكل مصدر اعتماد (طلبات · تلقائي · يدويّ)،
   وتحتها «x1.5» و«x2». وخلايا المجموعة **مدموجة**، فالعمود الثاني من كل
   مجموعة يبدو بلا اسم في الصفّ الأعلى.

   وقارئٌ يأخذ صفًّا واحدًا يفقد المعاملات كلَّها — فتُقرأ الدقائق صفرًا
   بثقةٍ كاملة، وهو أسوأ ما يمكن أن يحدث في هذا الملفّ.

   والشرط ضيّقٌ عمدًا: كل خليّة غير فارغة في الصفّ الثاني يجب أن تكون
   معاملًا، وواحدةٌ منها على الأقلّ. فصفُّ بيانات — فيه رقمٌ أو اسم — لا
   يُبتلع ترويسةً بحال. */
const MULTIPLIER_CELL_RE = /^[x×]\s*[0-9]+(?:[.,][0-9]+)?$/i;
...
function isSubHeaderRow(row) {
  ...
  for (const cell of row) {
    ...
    if (!text) continue;
    if (!MULTIPLIER_CELL_RE.test(text)) return false;
    sawMultiplier = true;
  }
  return sawMultiplier;
}

/* «اجمالي الاضافي من الطلبات» + «x1.5» ⇒ ترويسةٌ واحدة تحمل الاثنين.
   والعمود الذي لا اسم له في الأعلى (لأن خليّته مدموجة) يأخذ معاملَه
   وحده — وهو كافٍ: المعامل هو ما يُحتسب به، والمجموعةُ للعرض. */
function mergeHeaderRows(top, sub) {
  const width = Math.max(top.length, sub.length);
  const out = [];
  for (let i = 0; i < width; i += 1) {
    const a = String(top[i] === null || top[i] === undefined ? "" : top[i]).trim();
    const b = String(sub[i] === null || sub[i] === undefined ? "" : sub[i]).trim();
    out.push(b ? (a ? `${a} ${b}` : b) : a);
  }
  return out;
}
...
function findHeader(rows, kind) {
  ...
  if (isSubHeaderRow(sub)) {
    return {
      header: mergeHeaderRows(rows[best.index] || [], sub),
      dataRows: rows.slice(best.index + 2),
...
function parse({ kind, bytes }) {
  ...
  const { header, dataRows, headerRow, headerRows } = findHeader(table, kind);
...
module.exports = {
  ...
  isSubHeaderRow, mergeHeaderRows,
  ALIASES, EXPECTED_FIELDS, DEFAULT_FIELDS, CONFIDENCE_FLOOR, KIND_OVERTIME,
};
