...
function normalizeRows(kind, header, dataRows) {
  ...
  header.forEach((cell, index) => {
    ...
    if (!field) {
      /* عمودٌ لم يُتعرَّف عليه ليس خطأً بالضرورة — قد يكون «ملاحظات» أو
         «القسم». لكنه يُعلَن: أخطر ما في القراءة عمودٌ مهمّ سُمّي تسميةً
         لم تُدرَج، فيصير صفرًا صامتًا لا فراغًا مرئيًّا. */
  ...
  const weigh = (fields, weight) =>
    fields.reduce((acc, f) => ({
      got: acc.got + (f in map ? weight : 0), max: acc.max + weight,
    }), { got: 0, max: 0 });
  ...
  return {
    ...
    meta: {
      ...
      unknownColumns,
      confidence,
      needsManualReview: confidence < CONFIDENCE_FLOOR || spec.required.some((f) => !(f in map)),
    },
  };
}
...
module.exports = {
  ...
  available: true,
  parse,
  normalizeRows, normalizeHeader, toNumber, lastFour, splitRows,
