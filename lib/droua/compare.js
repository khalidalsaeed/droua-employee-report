  ...
  },
  {
    ...
    run({ overtime, divisors }) {
      ...
      for (const [empNo, entry] of overtime) {
        ...
        const minutes = Object.values(entry.minutes).reduce((t, m) => t + m, 0);
        if (!minutes) continue;
        out.push({
          employeeRef: empNo, employeeName: entry.name, field: "overtimeDivisor",
          title: "لا قاسمَ ساعةٍ محدَّدًا لهذا الموظّف",
          currentValue: `${(minutes / 60).toFixed(2)} ساعة`,
          description: "قيمةُ عمله الإضافيّ **لم تُحسب**: القاسم (8 أو 10) سياسةٌ "
            + "تُحدَّد ولا تُستنتج من الملفّ. حدّده في إعدادات الشهر ليُقيَّم.",
        });
      }
      return out;
    },
  ...
  },
  {
    ...
    run({ full, overtime, divisors }) {
      ...
      for (const [empNo, entry] of overtime) {
        ...
        const divisor = divisors.get(empNo);
        if (!divisor) continue;                    // تمسكها ot_no_divisor
        if (row.otAmount === null || row.otAmount === undefined) continue;
