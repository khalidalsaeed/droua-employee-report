...
const FIELD_LABELS = {
  ...
  otAmount: "بند العمل الإضافي", overtimeDivisor: "قاسم الساعة",
};
...
const RULE_LABELS = {
  ...
  ot_not_in_payroll: "عملٌ إضافيّ بلا راتب",
  ot_missing_source: "مبلغُ عملٍ إضافيّ بلا مصدر",
  ot_no_divisor: "لا قاسمَ ساعةٍ للموظّف",
  ot_multiplier_unsupported: "معاملٌ بلا صيغة",
  ot_unmatched_row: "صفوفٌ بلا رقمٍ وظيفيّ",
  ot_duplicate_entry: "صفٌّ مكرّر في العمل الإضافي",
  ot_amount_mismatch: "قيمةُ العمل الإضافي تخالف الساعات",
};
...
async function analyzeRun(sql, runId, ctx = {}) {
  ...
  /* إعداداتُ الشهر تدخل المحرّك معه: قاسمُ الساعة لكل موظّف، وتغطيةُ
     إجازته في هذا الشهر بعينه. وكلاهما من يد المستخدم — فما لم يُدخل
     يُعلَن ولا يُخمَّن. */
  const [divisors, leaveRows] = await Promise.all([
    settings.divisorMap(sql),
    settings.listLeaves(sql, runId),
  ]);
  const leaves = settings.leaveCoverage(leaveRows, run.period);

  const { findings: produced, applied, skipped, notEvaluable } =
    compare({ docs: current.docs, previousDocs, settings: { divisors, leaves } });
  const all = [
