...
/* كل فتحٍ ناجح يُنشئ صفًّا ولا يحذفه شيء، فالجدول ينمو بلا حدّ. التقليم
   يركب على نداءٍ قائم كتقليم المحاولات — بلا كرون ولا خانة إضافية في
   الخطّة.

   الحذف آمن: توكنٌ يشير إلى صفٍّ محذوف لا يجد جلسته فيُردّ مقفولًا. أي أن
   التقليم يفشل مغلقًا لا مفتوحًا. والمهلة يومٌ بعد السقف المطلق، فيبقى
   أثر الجلسات الحديثة متاحًا للفحص. */
const SESSION_PRUNE_GRACE_MS = 24 * 60 * 60 * 1000;

async function prune(sql, now) {
  await sql`
    DELETE FROM droua_gate_sessions
    WHERE absolute_exp < ${iso(now - SESSION_PRUNE_GRACE_MS)}::timestamptz`;
}

module.exports = {
  create, load, isUsable, touch, revoke, revokeAllForUser, prune,
  IDLE_MS, ABSOLUTE_MS, SESSION_PRUNE_GRACE_MS,
};
