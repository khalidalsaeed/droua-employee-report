...
async function activeSession(sql, req, user, cfg, now) {
  ...
  const session = await gateSessions.load(sql, gateToken.hashSid(payload.sid));
  ...
  return { payload, session, idleExp, renewed };
}

function setRenewed(res, live) {
  const maxAge = Math.max(0, Math.floor((live.renewed.exp - Date.now()) / 1000));
  res.setHeader("Set-Cookie", cookie(live.renewed.token, maxAge));
}
...
async function gateUnlock(req, res, gate) {
  ...
  const now = Date.now();
  const lock = await rateLimit.status(sql, user.id, now);
  if (lock.locked) {
  ...
  if (!gatePassword.verify(password, cfg.passwordHash, cfg.pepper)) {
    await rateLimit.recordFailure(sql, user.id, Date.now());
    const after = await rateLimit.status(sql, user.id, Date.now());
    /* العتبة الثالثة (15 خلال 24 ساعة) تُبلّغ ولا تُطيل القفل: أقصى قفل
       يبقى 60 دقيقة عمدًا — القسم لمستخدم واحد، وقفلُ يومٍ كامل خطرٌ
       تشغيليّ يفوق ما يضيفه فوق الطبقات القائمة. */
    return fail("gate_unlock_failed", { reason: "bad_password", critical: after.critical });
  }
  ...
  await rateLimit.clearFailures(sql, user.id);
  ...
  await floor(startedAt);
  H.secureApi(res);
  res.setHeader("Set-Cookie", cookie(issued.token, Math.floor((issued.exp - opened) / 1000)));
...
async function route(req, res) {
  ...
  if (!handler) return H.notFoundApi(res);
