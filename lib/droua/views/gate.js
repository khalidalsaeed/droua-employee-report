...
f.addEventListener('submit',function(e){
  ...
  fetch('/secure-audit/api/gate/unlock',{
    ...
    body:JSON.stringify({password:p.value})
      ...
      /* 401 = جلسة المنصّة انتهت، لا كلمة مرورٍ خاطئة. وعرضُ «تعذّر فتح
         القسم» هنا يقول للمستخدم إن كلمته خطأ فيعيدها ويعيدها — حتى يقفله
         حدُّ المحاولات على خطأٍ لم يرتكبه. وإعادةُ التحميل تُخرج تحويلًا
         إلى صفحة الدخول من الخادم نفسه. */
      if(r.status===401){ m.textContent='انتهت جلستك. يُعاد توجيهك لتسجيل الدخول…'; location.reload(); return null }
