...
var t=$('t'),m=$('m'),load=$('load'),until=0,runId=null,statuses=[],showAll=false,busy=0,totalRules=0;
...
function coverage(list){
  var el=$('cov');
  if(!totalRules){el.textContent='';el.className='cov';return}
  ...
  if(!n){el.className='cov';el.textContent='تم تقييم '+totalRules+' من '+totalRules+' قاعدة — التحليل كامل.';return}
  el.className='cov gap';
  el.textContent='تم تقييم '+(totalRules-n)+' من '+totalRules+' قاعدة — '+
    (n===1?'قاعدة واحدة لم يمكن تقييمها':n===2?'قاعدتان لم يمكن تقييمهما':n+' قواعد لم يمكن تقييمها')+
