...
<section id="v-run" hidden>
  ...
  <p class="note err" id="cfg-fail" hidden></p>
  <details id="cfg" class="cfgbox">
    ...
    <div class="cfgbody">
      ...
      <p class="row">
        <input id="lv-find" type="search" placeholder="ابحث بالاسم أو الرقم" aria-label="بحث عن موظّف">
        <select id="lv-emp" aria-label="الموظّف"></select>
        <input id="lv-no" type="text" inputmode="numeric" placeholder="الرقم الوظيفي" aria-label="الرقم الوظيفي" hidden>
      ...
      <p class="small muted" id="lv-fallback" hidden>تعذّر تحميل قائمة الموظفين —
        اكتب الرقم الوظيفي يدويًّا.</p>
      <div class="scroll"><table>
      ...
      <div class="scroll"><table>
        <thead><tr><th>الموظّف</th><th>الرقم</th><th>دقائق العمل الإضافي</th><th>القاسم</th></tr></thead>
        <tbody id="dv-list"></tbody>
      </table></div>
      <p class="empty small" id="dv-empty" hidden></p>
      <p class="row">
        <button id="dv-save" class="ghost" type="button">حفظ الكل</button>
        <span class="small muted" id="dv-note"></span>
      </p>
      <details id="dv-manual">
        <summary class="small muted">إدخالٌ يدويّ لموظّفٍ ليس في الملفّ</summary>
        <p class="row">
  ...
  <div class="totals" id="totals" hidden>
    <h4>إجماليّا الصرف</h4>
    <div class="tgrid">
      <div class="tcard"><span class="tlabel">تحويل بنكيّ</span>
        <span class="tval" id="t-bank">—</span>
        <span class="tsub small muted" id="t-bank-n"></span></div>
      <div class="tcard"><span class="tlabel">نقدًا</span>
        <span class="tval" id="t-cash">—</span>
        <span class="tsub small muted" id="t-cash-n"></span></div>
    </div>
    <p class="small muted">مجموعُ ما قُرئ من صفوف الموظفين — لا صفّ المجاميع في الملفّ.</p>
  </div>
</section>
...
function openRun(id){
  ...
  return act('جارٍ تحميل الشهر…',function(){
    return api('/runs/'+id).then(function(j){
      ...
      bindSlots();
      view('run');
      /* الإعداداتُ لوحةٌ اختيارية، والملاحظاتُ والإجماليّاتُ هما سببُ
         الصفحة. فسقوطُ الأولى يُطوى في «تعذّرت» ولا يُسقط ما بعدها —
         وإلّا فُتح الشهرُ على صفحةٍ فارغة. */
      return loadSettings().catch(function(e){
        show($('cfg-fail'),true);
        $('cfg-fail').textContent='تعذّر تحميل إعدادات الشهر ('+(e&&e.message||'')+') — '
          +'الملاحظاتُ والإجماليّاتُ أدناه سليمة.';
      }).then(loadFindings).then(loadTotals);
    });
  });
}
function bindSlots(){
...
/* ── إجماليّا الصرف ── */
/* «123,456.78 ر.س» — فاصلٌ ألفيّ ومنزلتان دائمًا. والصفرُ المضلّل ممنوع:
   ما لم يُقرأ يُقال «غير متاح» لا «0.00». */
function money(n){
  var v=(Math.round(Number(n||0)*100)/100).toFixed(2).split('.');
  return v[0].replace(/\\B(?=(\\d{3})+(?!\\d))/g,',')+'.'+v[1]+' ر.س';
}
/* «6 موظّفًا» خطأٌ يقرؤه المستخدم كما قرأ «20 قواعد»: من ثلاثةٍ إلى عشرة
   جمعُ قلّة، ومن أحدَ عشرَ فصاعدًا مفردٌ منصوب. */
function countEmp(n){
  if(n===0)return 'لا أحد';
  if(n===1)return 'موظّفٌ واحد';
  if(n===2)return 'موظّفان';
  if(n<=10)return n+' موظّفين';
  return n+' موظّفًا';
}
function loadTotals(){
  return api('/runs/'+runId+'/totals').then(function(j){
    var any=(j.transfer&&j.transfer.available)||(j.cash&&j.cash.available);
    show($('totals'),true);
    [['transfer','t-bank'],['cash','t-cash']].forEach(function(pair){
      var d=j[pair[0]]||{};
      $(pair[1]).textContent=d.available?money(d.total):'غير متاح';
      $(pair[1]).className='tval'+(d.available?'':' muted');
      $(pair[1]+'-n').textContent=d.available?countEmp(d.count):'الملفّ غير مرفوع أو تعذّرت قراءته';
    });
    return any;
  }).catch(function(){show($('totals'),false)});
}

/* ── إعدادات الشهر ── */
/* قائمةُ موظفي الشهر ومن له عملٌ إضافيّ — تُقرأ من الملفّات المرفوعة.
   الرقم الوظيفيّ يبقى المفتاح، والاسمُ للعرض والبحث وحده. */
var roster=[],otRows=[];

/* تصفيةٌ بالاسم أو الرقم. والمقارنة على النصّ الخام: الاسم عربيّ، وأي
   «تطبيع» ذكيّ هنا يُخفي موظّفًا عن من يبحث عنه باسمه كما يكتبه. */
function fillEmployeeSelect(){
  var q=($('lv-find').value||'').trim();
  var sel=$('lv-emp'); var keep=sel.value;
  var list=roster.filter(function(e){
    if(!q)return true;
    return String(e.empNo).indexOf(q)>=0 || (e.name&&e.name.indexOf(q)>=0);
  }).slice(0,300);
  sel.textContent='';
  list.forEach(function(e){
    var o=document.createElement('option');
    o.value=e.empNo;
    o.textContent=(e.name?e.name+' — ':'')+e.empNo;
    sel.appendChild(o);
  });
  /* يُبقي الاختيار قائمًا إن بقي ضمن المصفّى — فالكتابة لا تُضيّع ما اختير. */
  if(keep&&list.some(function(e){return e.empNo===keep}))sel.value=keep;
}

/* الرقم الوظيفيّ المختار: من القائمة، أو من الإدخال اليدويّ حين تتعذّر. */
function chosenEmpNo(){
  if($('lv-no').hidden===false)return ($('lv-no').value||'').trim();
  return $('lv-emp').value||'';
}

function renderOvertimeRows(){
  var b=$('dv-list'); b.textContent='';
  otRows.forEach(function(e){
    var tr=document.createElement('tr');
    var opts='<option value="8"'+(e.overtimeDivisor===8?' selected':'')+'>8 ساعات</option>'+
      '<option value="10"'+(e.overtimeDivisor===10?' selected':'')+'>10 ساعات</option>';
    /* الافتراضيّ مختارٌ سلفًا فلا يُسأل عنه أحد، ويُميَّز عن قرارٍ محفوظ:
       القيمة واحدة، ومصدرُها ليس واحدًا. */
    var src=e.isDefault?'<div class="small muted">افتراضيّ</div>':'<div class="small muted">محفوظ</div>';
    tr.innerHTML='<td>'+esc(e.name||'—')+'</td><td>'+esc(e.empNo)+'</td>'+
      '<td>'+esc(e.minutes)+' د ('+(e.minutes/60).toFixed(2)+' س)</td>'+
      '<td><select class="dv-pick" data-no="'+esc(e.empNo)+'">'+opts+'</select>'+src+'</td>';
    b.appendChild(tr);
  });
  var none=!otRows.length;
  $('dv-empty').textContent=otLoaded
    ?'لا موظّف له عملٌ إضافيّ أكبر من صفر في هذا الشهر.'
    :'لم يُرفع ملفّ العمل الإضافي بعد.';
  show($('dv-empty'),none);
  show($('dv-save'),!none);
  var byDefault=otRows.filter(function(e){return e.isDefault}).length;
  $('dv-note').textContent=none?''
    :(byDefault?byDefault+' منهم بالقاسم الافتراضيّ — عدّله لمن يستثنيه نظامُ الشركة ثمّ احفظ.'
      :'كلُّهم بقاسمٍ محفوظ.');
}
...
function loadSettings(){
  return api('/runs/'+runId+'/settings').then(function(j){
    ...
    (j.leaves||[]).forEach(function(x){
      var tr=document.createElement('tr');
      /* الاسمُ من القائمة — والرقم يبقى المعروض معه، فهو المفتاح. */
      var who=(roster.find(function(e){return e.empNo===x.empNo})||{}).name;
      tr.innerHTML='<td>'+esc(who||'—')+'<div class="small muted">'+esc(x.empNo)+'</div></td>'+
        '<td><input class="lv-f" type="date" value="'+esc(x.startDate)+'" aria-label="من"></td>'+
        '<td><input class="lv-t" type="date" value="'+esc(x.endDate)+'" aria-label="إلى"></td>'+
        '<td><button class="ghost lv-edit" data-id="'+esc(x.leaveId)+'" type="button">حفظ</button> '+
        '<button class="ghost lv-del" data-id="'+esc(x.leaveId)+'" type="button">حذف</button></td>';
      lv.appendChild(tr);
    });
    show($('lv-empty'),!(j.leaves&&j.leaves.length));

    roster=j.employees||[];
    otRows=j.overtimeEmployees||[];
    otLoaded=!!j.overtimeLoaded;
    /* تعذّرت القائمة ⇒ الإدخال اليدويّ بديلًا، لا شاشةٌ معطّلة. */
    var manual=!j.rosterLoaded||!roster.length;
    show($('lv-fallback'),manual);
    $('lv-no').hidden=!manual;
    $('lv-emp').hidden=manual;
    $('lv-find').hidden=manual;
    if(!manual)fillEmployeeSelect();
    renderOvertimeRows();
    bindSettings();
  });
}
function bindSettings(){
  [].forEach.call(document.querySelectorAll('.lv-edit'),function(el){
    el.addEventListener('click',function(){
      var row=el.closest('tr');
      var from=row.querySelector('.lv-f').value,to=row.querySelector('.lv-t').value;
      if(!from||!to){err('أدخل التاريخين.');return}
      if(to<from){err('نهاية الإجازة قبل بدايتها.');return}
      act('جارٍ الحفظ…',function(){
        return api('/leaves/'+el.dataset.id,{method:'PATCH',body:{startDate:from,endDate:to}})
          .then(function(){return loadSettings().then(function(){ok('عُدِّلت الإجازة. أعد التحليل ليُحتسب أثرها.')})});
      }).catch(function(){});
    });
  });
...
$('lv-add').addEventListener('click',function(){
  var no=chosenEmpNo(),from=$('lv-from').value,to=$('lv-to').value;
  if(!no||!from||!to){err('اختر الموظّف وحدّد تاريخَي الإجازة.');return}
  ...
  act('جارٍ الحفظ…',function(){
    return api('/runs/'+runId+'/leaves',{method:'POST',body:{empNo:no,startDate:from,endDate:to}})
      .then(function(){$('lv-no').value='';$('lv-find').value='';return loadSettings().then(function(){ok('أُضيفت الإجازة. أعد التحليل ليُحتسب أثرها.')})});
  }).catch(function(){});
});
$('lv-find').addEventListener('input',fillEmployeeSelect);
$('dv-save').addEventListener('click',function(){
  var picks=[];
  [].forEach.call(document.querySelectorAll('.dv-pick'),function(el){
    picks.push({empNo:el.dataset.no,overtimeDivisor:Number(el.value)});
  });
  if(!picks.length){err('لم تُحدَّد قواسم لتُحفظ.');return}
  act('جارٍ الحفظ…',function(){
    return api('/settings/divisors/bulk',{method:'PUT',body:{divisors:picks}})
      .then(function(r){return loadSettings().then(function(){
        ok('حُفظ '+((r&&r.saved)||picks.length)+' قاسمًا. أعد التحليل ليُحتسب.')})});
  }).catch(function(){});
});
