...
function renderOvertimeRows(){
  ...
  otRows.forEach(function(e){
    ...
    var opts='<option value="">— غير محدَّد —</option>'+
      '<option value="8"'+(e.overtimeDivisor===8?' selected':'')+'>8 ساعات</option>'+
    ...
    tr.innerHTML='<td>'+esc(e.name||'—')+'</td><td>'+esc(e.empNo)+'</td>'+
      ...
      '<td><select class="dv-pick" data-no="'+esc(e.empNo)+'">'+opts+'</select></td>';
    /* غيرُ المحدَّد يُعلَّم بوضوح: قيمتُه لا تُحسب حتى يُحدَّد. */
    if(!e.overtimeDivisor)tr.className='na';
    b.appendChild(tr);
  });
  ...
  show($('dv-empty'),none);
  show($('dv-save'),!none);
  var missing=otRows.filter(function(e){return !e.overtimeDivisor}).length;
  $('dv-note').textContent=none?''
    :(missing?missing+' موظّفًا بلا قاسم — قيمتُهم لن تُحسب.':'كلُّهم محدَّدون.');
}
...
$('dv-save').addEventListener('click',function(){
  ...
  [].forEach.call(document.querySelectorAll('.dv-pick'),function(el){
    if(el.value)picks.push({empNo:el.dataset.no,overtimeDivisor:Number(el.value)});
  });
