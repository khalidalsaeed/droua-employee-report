...
/* ── إجماليّا الصرف: بطاقتان أسفل الصفحة، لا صفٌّ ثالث في جدول الملاحظات ──
   خلطُهما بالملاحظات يجعل رقمًا إخباريًّا يبدو ملاحظةً تستدعي فعلًا. */
.totals{margin:22px 0 4px;padding-top:16px;border-top:1px solid var(--line)}
.totals h4{margin:0 0 10px;font-size:.9rem;font-weight:600}
.tgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px}
.tcard{border:1px solid var(--line);border-radius:11px;padding:12px 14px;background:var(--bg)}
.tlabel{display:block;font-size:.8rem;color:var(--muted)}
.tval{display:block;font-size:1.25rem;font-weight:600;margin:4px 0 2px;
  font-variant-numeric:tabular-nums;letter-spacing:-.01em}
.tval.muted{font-size:1rem;font-weight:500}
.tsub{display:block}
