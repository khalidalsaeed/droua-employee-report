...
/* ── القاسم الافتراضيّ ──
   سياسةُ الشركة: عشر ساعات للجميع إلا فئةً محدودة تُستثنى بثمانٍ. فبدل أن
   يُسأل المستخدم عن كل موظّف كل شهر، يُطبَّق العشرة تلقائيًّا على من له
   عملٌ إضافيّ ولا صفَّ محفوظ له — والمستثنون صفوفٌ محفوظة تُكتب مرّة.

   ولماذا في القاعدة لا في الشيفرة: أسماءُ المستثنين وأرقامهم **بيانُ
   موظفين**، ولا يدخل Git. والقائمةُ تتغيّر بقرارٍ إداريّ لا بإصدارِ
   نشرة — فمكانُها جدولُ الإعدادات القائم، ويُغيَّر من الشاشة. */
const DEFAULT_OVERTIME_DIVISOR = 10;
...
/* حفظُ دفعةٍ واحدة: الشاشة تعرض موظفي العمل الإضافي كلَّهم وتحفظهم معًا.
   والكتابة **واحدةً بعد واحدة** هنا مقصودة: العدد صغير (من له عملٌ إضافيّ
   في الشهر)، والعبارة نفسها المُختبَرة تُعاد استعمالها — فلا صيغة جديدة
   تُكتب لمسارٍ يُغيّر مبلغًا متوقَّعًا. */
async function setDivisors(sql, entries, ctx = {}) {
  gateContext.requireGate();
  const list = Array.isArray(entries) ? entries : [];
  if (!list.length) throw fail("لا شيء ليُحفظ", "empty_batch");
  if (list.length > 500) throw fail("دفعةٌ أكبر من الحدّ", "batch_too_large");
  /* يُتحقّق من **الجميع** قبل كتابة أيٍّ منهم: دفعةٌ نصفُها مكتوب ونصفُها
     مرفوض تترك المستخدم لا يدري ما حُفظ. */
  const clean = list.map((e) => {
    const divisor = Number(e && e.overtimeDivisor);
    if (!DIVISORS.includes(divisor)) throw fail("القاسم إمّا 8 أو 10", "bad_divisor");
    return { empNo: assertEmpNo(e && e.empNo), overtimeDivisor: divisor };
  });
  const seen = new Set();
  for (const e of clean) {
    if (seen.has(e.empNo)) throw fail("رقمٌ مكرّر في الدفعة", "duplicate_emp_no");
    seen.add(e.empNo);
  }
  for (const e of clean) await setDivisor(sql, e, ctx);
...
/* تعديلُ فترةٍ قائمة بدل حذفها وإعادتها: المعرّف يبقى، فيبقى أثرُ من
   أضافها ومتى. */
async function updateLeave(sql, leaveId, { startDate, endDate }) {
  gateContext.requireGate();
  ...
  const to = assertDate(endDate);
  if (to < from) throw fail("نهاية الإجازة قبل بدايتها", "bad_range");
  const rows = await sql`UPDATE droua_run_leaves
    SET start_date = ${from}::date, end_date = ${to}::date
    WHERE id = ${id}::uuid RETURNING id`;
  if (!rows || !rows.length) throw fail("غير موجودة", "not_found");
  return { leaveId: id, startDate: from, endDate: to };
}
...
module.exports = {
  DIVISORS, DEFAULT_OVERTIME_DIVISOR, MAX_NOTE,
  listDivisors, divisorMap, setDivisor, setDivisors, clearDivisor,
  listLeaves, addLeave, updateLeave, removeLeave,
