...
const protectedUsers = require("./protectedUsers");
...
async function updateUser(id, patch, opts = {}) {
  /* حارس الحساب المحميّ — هنا في طبقة البيانات عمدًا، لا في الموزّع.
     السبب أن lib/data/registry.js يحمل مسارًا ثانيًا إلى هذه الدالّة عبر
     usersAdapter؛ هو غير قابل للبلوغ اليوم لأن handleData يحوّل "users"
     إلى handleUsers قبل بلوغ getResource، لكنه موجود — وسطرٌ يُعاد ترتيبه
     مستقبلًا يفتحه. الحارس هنا يغطّي كل مُنادٍ: الموزّع، والـadapter، وأي
     سكربت أو شيفرة تُكتب بعد سنة.

     api/app.js يفحص القواعد نفسها قبل النداء ليردّ 403 برسالة واضحة —
     لكن الضمان هذا، لا ذاك. */
  const refusal = protectedUsers.guardUpdate(id, patch, opts);
...
async function deleteUser(id) {
  /* الحذف لا استثناء له: لا صاحب الحساب ولا غيره. (حذف الذات محروس أصلًا
     في api/app.js، وهذا يغطّي كل المسارات الأخرى.) */
  const refusalDelete = protectedUsers.guardDelete(id);
  if (refusalDelete) throw new Error(refusalDelete.message);
  const sql = getSql();
  const rows = await sql`DELETE FROM users WHERE id = ${id} RETURNING id`;
  if (!rows[0]) throw new Error("المستخدم غير موجود");
}

/* يمرّ بحارس الحساب المحميّ بسلام: lastLogin مصنَّف حقلًا نظاميًا، فتسجيل
   الدخول لا ينكسر على حساب محميّ. */
