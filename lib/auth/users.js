...
function envOwnerUser() {
  ...
  return { id: ENV_OWNER_ID, name: "Owner", email, passwordHash, role: "owner", status: "active", createdAt: null, lastLogin: null };
}
...
function envHrSpecialistUser() {
  ...
  if (!email || !passwordHash) return null;
  return { id: ENV_HR_SPECIALIST_ID, name: process.env.HR_SPECIALIST_NAME || "HR Specialist", email, passwordHash, role: "hr", status: "active", createdAt: null, lastLogin: null };
}
function envUsers() {
...
async function createUser({ name, email, passwordHash, role, status }) {
  ...
  const rows = await sql`
    ...
    VALUES (${id}, ${name}, ${email}, ${passwordHash}, ${role}, ${status || "active"})
...
async function updateUser(id, patch) {
  ...
  const rows = await sql`
    UPDATE users SET name = ${merged.name}, email = ${merged.email}, password_hash = ${merged.passwordHash},
      role = ${merged.role}, status = ${merged.status}, last_login = ${merged.lastLogin || null}
    WHERE id = ${id}
    RETURNING *`;
  return stripSecrets(toUser(rows[0]));
}
