...
/* Which page/section keys each role may see. Add a new page key here (and to
   the pages actually gated in app-shell.html / users-shell.html) to extend the system. */
...
/* Owner-protection rule only: an Owner account may be managed by an Owner and
   nobody else. WHETHER the actor may manage users at all is decided earlier by
   the users:* permissions (lib/auth/permissions.js) — this used to also encode
   "must be owner/admin", which would have made a permission grant like
   users:manage unusable for any other role. */
