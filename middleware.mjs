...
const PUBLIC_PATHS = new Set([
  ...
  /* عامل الخدمة عامّ عمدًا. المتصفّح يُعيد جلب سكربته دوريًا للتحقّق من
     التحديث، وقد يقع ذلك والجلسة منتهية؛ فيردّ هذا الوسيط بتحويل 302 إلى
     صفحة الدخول — والتحويل على سكربت عامل الخدمة خطأ قاطع يُبطل العامل
     المسجَّل وتسقط الإشعارات صمتًا. الملف لا يحوي بيانات ولا أسرارًا:
     معالجا push و notificationclick فقط، بلا تخزين مؤقّت وبلا حدث fetch. */
  "/sw.js",
]);
