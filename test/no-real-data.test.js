...
/* أرقام حسابات مُختلقة في عيّنات الإيصالات. مُعلَنة واحدًا واحدًا لا
   بنمطٍ متسامح: قائمةٌ صريحة تُراجَع عند كل إضافة. */
const FAKE_ACCOUNTS = new Set([
  "010800000000000099", // test/fixtures/receipts.js — حساب المُرسِل
  "12345678901234",     // test/payroll-receipt-parser.test.js — حساب المستفيد بوسمه
  "55555555555555",     // نفسه، بالوسم العربي
  "99999999999999",     // رقم آخر في الصفحة، يجب ألّا يُنسَب للمستفيد
]);
...
/* IBANات العيّنات المُعقَّمة تبدأ بـSA9 — والحقيقية السعودية SA02،
   فمدى SA9x محجوز للاختلاق ولا يمكن أن يقابل حسابًا قائمًا.

   والاستثناء **مشروط بالنمط لا بالملفّ**: دسُّ IBAN حقيقي في عيّنة
   يُسقط الاختبار، لأن الشرط أن يبدأ كل IBAN في تلك الملفّات بـSA9. */
const FAKE_IBAN_RE = /^SA9\d{21}$/;

test("لا IBAN سعودي حقيقي في أي ملفّ نصّي", () => {
  const IBAN = /SA\d{22}/g;
  const hits = [];
  for (const f of TEXT_FILES) {
    for (const m of fs.readFileSync(f, "utf8").matchAll(IBAN)) {
      if (FAKE_IBAN_RE.test(m[0])) continue;
      hits.push(`${rel(f)} → ${m[0].slice(0, 6)}…`);
    }
  }
  assert.deepEqual(hits, [], `ما يشبه IBAN حقيقيًا (المُختلَق يبدأ بـSA9):\n  ${hits.join("\n  ")}`);
});

test("لا رقم حساب بنكي طويل في شيفرتنا", () => {
  /* ثماني عشرة إلى عشرين خانة — وهو طول أرقام الحسابات في هذه المنصّة
     (قِيس: 010809…0014). والحدّان مضبوطان على ما نحرسه فعلًا:

     · أقلّ من 18: رقم الهاتف السعودي الدولي اثنتا عشرة خانة وهو بيان
       شركة ظاهر في الواجهة، ومقاطع IBAN المقطوع في العيّنات المُعقَّمة
       أربع عشرة إلى ست عشرة خانة. فحدٌّ أدنى عند 14 كان يمسكها كلها
       ويُغرق الحاجز بضجيج يُطفئه أحدٌ يومًا.
     · وIBAN نفسه (24 محرفًا) له فحصه الخاصّ أعلاه، فلا يُفلت. */
  const ACCOUNT = /(?<!\d)\d{18,20}(?!\d)/g;
  const hits = [];
  for (const f of TEXT_FILES) {
    if (LOCKFILES.has(path.basename(f))) continue;
    for (const m of fs.readFileSync(f, "utf8").matchAll(ACCOUNT)) {
      if (FAKE_ACCOUNTS.has(m[0])) continue;
      hits.push(`${rel(f)} → ${m[0].slice(0, 4)}… (${m[0].length} رقمًا)`);
    }
  }
